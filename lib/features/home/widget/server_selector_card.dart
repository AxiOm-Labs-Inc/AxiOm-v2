...
class _ServerSelectorBody extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Widget buildTransportRow(WidgetRef ref) {
      // Hysteria2 (QUIC), Naive (HTTP/2) и ShadowTLS — у каждого один транспорт, строку прячем.
      if (protocol == ServerOption.protocolHysteria2 ||
          protocol == ServerOption.protocolNaive ||
          protocol == ServerOption.protocolShadowTls) {
      ...
      if (isAuto) {
        if (fastestOption == null ||
            ...
            fastestOption!.protocol == ServerOption.protocolShadowTls) {
          return const SizedBox.shrink();
        }
